...
def eval(parameters):
    ...
    for gallery_file in tqdm(gallery_files, total=len(gallery_files), leave=True, mininterval=0.1, ascii=True,
                             ncols=NCOLS):
    ...
    for probe_file, GT_id in tqdm(zip(probe_files, probe_ids), total=len(probe_ids), leave=True, mininterval=0.1,
                                  ascii=True, ncols=NCOLS):
        evaled_cnt += 1
        ...
        #
        sorted_gallery_ids = gallery_ids_np[0, sort_scores_idx[0]]
        #
        top1_id = [sorted_gallery_ids[-1]]
        top5_ids = list(sorted_gallery_ids[-5:])
        #
        top1_cnt += 1 if GT_id in top1_id else 0
        top5_cnt += 1 if GT_id in top5_ids else 0

    writer.add_scalar(tag='eval/top1_acc', scalar_value=top1_cnt / evaled_cnt, global_step=parameters['global_step'])
    writer.add_scalar(tag='eval/top5_acc', scalar_value=top5_cnt / evaled_cnt, global_step=parameters['global_step'])

    return top1_cnt / evaled_cnt, top5_cnt / evaled_cnt
