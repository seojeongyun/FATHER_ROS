...
from models.efficientcaps import efficientcapsnet, efficientrescaps
...
class GroupFace_caps(nn.Module):
    def __init__(self,
                 data_h=224, data_w=224,
                 capdimen=24, predcapdimen=32, numpricap=1024,
                 num_final_cap=128,
                 feature_dim=512, groups=4, mode='S'):
        super(GroupFace_caps, self).__init__()
        self.mode = mode
        self.groups = groups
        self.Backbone = efficientrescaps(data_h, data_w, capdimen, predcapdimen, numpricap, num_final_cap)
