...
from YOLOv6.yolov6.utils.nms import non_max_suppression_face, non_max_suppression
...
class demo:
    ...
    def get_model(self, onnx, weight):
        if onnx:
            ...
            if device_type == 'cuda' and 'CUDAExecutionProvider' in available_providers:
                ...
                if device_type == 'cuda':
                    LOGGER.warning('CUDAExecutionProvider is unavailable. ''ONNX Runtime will use CPUExecutionProvider.')

            LOGGER.info(f'Loading ONNX model: {weight}, providers={providers}')

            return ort.InferenceSession(str(weight), providers=providers)

        from YOLOv6.yolov6.utils.torch_utils import fuse_model

        LOGGER.info(f'Loading PyTorch model: {weight}')
    ...
    def torch_run(self, imgs: list):
        ...
        resized_imgs, ratios, pads = letterbox(resized_imgs, new_shape=(self.resized_size, self.resized_size), auto=False, scaleup=False,)

        input_array = np.stack(resized_imgs, axis=0)
        ...
        if self.onnx:
            ort_outputs = self.model.run(self.output_names, {self.input_name: input_array},)
            outputs = torch.from_numpy(ort_outputs[0])
            inference_imgs = input_array
        else:
            inference_imgs = torch.from_numpy(input_array)
            inference_imgs = inference_imgs.to(self.device, non_blocking=True)
            outputs, _ = self.model(inference_imgs)
        ...
        if self.net_type == '6n':
            ...
            outputs = non_max_suppression_face(
                outputs,
                self.conf_thres,
                self.iou_thres,
                max_det=self.nms_det,
        ...
        rescaled_bboxes = self.rescale_bboxes(bboxes, inference_imgs, shapes)
    ...
    def post_processing(bboxes, imgs):
        ...
        for im_idx in range(len(imgs)):
            ...
            for bbox_idx in range(len(bboxes[im_idx])):
                ...
                cropped_face.append(img[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])])
                img = cv2.rectangle(img.copy(), (int(bbox[0]), int(bbox[1])),
                                    (int(bbox[2]), int(bbox[3])), (24, 252, 0), 2)
                img = cv2.putText(img, str(np.round(bbox[4].cpu().numpy(), 2)), (int(bbox[0]), int(bbox[1]) - 10),
                                  cv2.FONT_HERSHEY_COMPLEX, 0.5, (24, 252, 0), thickness=1)
            post_.append(img)
        return post_, cropped_face
